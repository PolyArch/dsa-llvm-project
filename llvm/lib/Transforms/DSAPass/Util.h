...
#define DEFINE_FLAG(x, default_) \
  ...
  /*!
   * \brief If we want to move all the processing elements on triggered PE (even if they are not annotated with temporal).
   */
  ...
  /*!
   * \brief If we want to execute the peephole optimization over dsa register file instructions.
   */
